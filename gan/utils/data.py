...
def get_csi_data_by_year(
    ...
    name = instruments + '_market_pkl_' + str(target).replace('/', '_').replace(' ', '') + '_' + freq
    name = f"{name}_{train_start}_{train_end}_{valid_start}_{valid_end}_{test_start}_{test_end}"
    try:
        data = load_pickle(f'pkl/{name}/data.pkl')
        data_valid = load_pickle(f'pkl/{name}/data_valid.pkl')
        ...
        print('Data not exist, load from qlib')
        data = get_data_my(instruments, train_start, train_end, raw=True, qlib_path=QLIB_PATH, freq=freq, csi=True)
        data_valid = get_data_my(instruments, valid_start, valid_end, raw=True, qlib_path=QLIB_PATH, freq=freq, csi=True)
        data_valid_withhead = get_data_my(instruments, valid_head_start, valid_end, raw=True, qlib_path=QLIB_PATH,freq=freq, csi=True)
        data_test = get_data_my(instruments, test_start, test_end, raw=True, qlib_path=QLIB_PATH, freq=freq, csi=True)
        data_test_withhead = get_data_my(instruments, test_head_start, test_end, raw=True, qlib_path=QLIB_PATH,freq=freq, csi=True)
    ...
    try:
        ...
        data_all = get_data_my(instruments, train_start, test_end, raw=True, qlib_path=QLIB_PATH, freq=freq, csi=True)
        save_pickle(data_all, f'pkl/{name}/data_all.pkl')
    return data_all, data, data_valid, data_valid_withhead, data_test, data_test_withhead, name
